...
def simulate_order_fills(manager, order, max_seconds=30):
    ...
    """
    print(f"\nSimulating fills for Order #{order.order_number}...")
    ...
    # Final status
    if order.is_filled:
        print(f"\nOrder #{order.order_number} completely filled in {time.time() - start_time:.1f} seconds")
        print(f"Average fill price: ${order.average_fill_price:.2f}")
    else:
        print(f"\nSimulation ended. Order #{order.order_number} is {order.filled_quantity}/{order.quantity} filled")
